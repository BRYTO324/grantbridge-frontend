...
import { LayoutDashboard, Compass, User, Settings, LogOut, X, MapPin, BookOpen } from "lucide-react";
...
export default function FunderSidebar({ isOpen, onClose }: FunderSidebarProps) {
  ...
  const displayName = user?.fullName || "Funder";
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <p className="px-3 pt-1 pb-2 text-xs font-bold text-slate-400 uppercase tracking-[0.08em]">Menu</p>
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                active ? "bg-brand-50 text-brand-700 shadow-sm shadow-brand-100/50" : "text-slate-500 hover:bg-slate-50 hover:text-slate-700"
              }`}
            >
        ...
        <p className="px-3 pt-5 pb-2 text-xs font-bold text-slate-400 uppercase tracking-[0.08em]">Account</p>
